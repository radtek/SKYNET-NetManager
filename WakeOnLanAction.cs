...
namespace WakeOnLanAction
{
    public static class LANHelper
    {
        ...
        internal static List<AddressStruct> ListAllHosts(byte[] ipBase)
        {
            List<AddressStruct> list = new List<AddressStruct>();
            for (int b4 = 0; b4 <= 255; b4++)
            {
                var ip = ipBase[0] + "." + ipBase[1] + "." + ipBase[2] + "." + b4;
                ...
                {
                    if (e.Error == null && e.Reply.Status == IPStatus.Success)
                        list.Add(new AddressStruct()
                        {
                            IPAddress = e.Reply.Address,
                            MacAddress = GetMacAddress(e.Reply.Address)
                        });
                };
                ping.SendAsync(ip, null);
                frmMain.frm.ProfileSelected.Text = ip;

            }
            return list;
        }
        private static PhysicalAddress GetMacAddress(System.Net.IPAddress address)
        {
