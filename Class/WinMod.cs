...
namespace SkynetManager
{
    public class WinMod
    {
        ...
        public static bool IsWindowMinimized(FormWindowState windowState)
        {
            return windowState == FormWindowState.Minimized;
        }

        public static bool IsActiveWindowWithoutIdle(long hw)
        {
            if (IsActiveWindow(hw))
                return true;
            else
                return false;
        }

        public static bool IsActiveWindow(long hw)
        {
            bool flag;

            if (frmMain.frm.WindowState == FormWindowState.Minimized)
            {
                flag = false;
                goto label_4;
            }

            flag = GetForegroundWindow() == (IntPtr)hw;
            label_4:
            return flag;
        }
        ...
        public static void SetActiveWindow(long hwnd)
        {
            ...
            {
                /*
                WinMod.MakeWindowTop(hwnd);
                WinMod.SetFocus(checked((int)hwnd));
                frmChat frmChat = (frmChat)Modcommon.GetChatForm(Modcommon.ACTIVECHAT_KEY);
                if (frmChat == null || !frmChat.Handle.ToString().Equals(hwnd.ToString()))
                    return;
                frmChat.SetSendFocus(false);
                */
            }
            catch (Exception ex)
            {
        ...
        public static void SetActiveWindow2(long hwnd)
        {
            try
            {
                MakeWindowTop(hwnd);
        ...
        public static TimeSpan? GetInactiveTime2()
        {
            LASTINPUTINFO plii = new LASTINPUTINFO();
            plii.cbSize = checked((uint)Marshal.SizeOf((object)plii));
            plii.dwTime = 0U;
